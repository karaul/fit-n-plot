	...
	for (var i = linestart; i < lines.length; i++) {
		...
		if (isNaN(parseFloat(s[0]))) {} else {
		  var x=parseFloat(s[0]);
	 	  ...
	 	  dataYErr.push( {x: x, y: [ parseFloat(s[1])-parseFloat(s[2]), parseFloat(s[1])+parseFloat(s[2]) ] });
...
fReaderHist.onload = function (e) {
    ...
    var chart = new CanvasJS.Chart("plothist", {
	   theme: "light2", 
	   title:{ text: "Parameter PDF"},	data: [{ type: "column", dataPoints: []},]
	});	
	
	for (var i = linestart; i < lines.length; i++) {
		var s = lines[i].split(delim[0]);
		if (isNaN(parseFloat(s[0]))) {} else {
			chart.options.data[0].dataPoints.push( { x: parseFloat(s[0]), y: parseFloat(s[1]) } );
		  //if( xStart > x  ) { xStart = x;}
		  //if( xEnd < x  ) { xEnd = x;}
		}
	}
...
document.getElementById('plot').onclick =  function () {
        ...
        for (var i = 0; i < Nbin+1; i++) {
		  ...
		  chart.options.data.push( { type: "line", showInLegend: true, legendText: "fit: "+eq, dataPoints: data });
          chart.render();
